...
- **增强：Ref 下拉加搜索**：分支/tag 多时下拉难以查找。ComboBox 改为 `IsEditable` + `StaysOpenOnEdit` + `IsTextSearchEnabled=False`，填充改用 `ObservableCollection` + `ItemsSource` + `CollectionView.Filter`；ComboBox.Loaded 时拿到内部 `PART_EditableTextBox` 订阅 `TextChanged` 实时过滤（Workspace 项始终保留，其余按 Display 包含搜索文本，忽略大小写）。下拉打开时清空过滤显示全部并全选文本框让输入直接覆盖；关闭时若用户输入搜索文本未点选，恢复文本框为当前选中项显示文本—— [StatisticsUserControl.xaml](file:///workspace/src/ForkPlus/UI/UserControls/StatisticsUserControl.xaml)、[StatisticsUserControl.xaml.cs](file:///workspace/src/ForkPlus/UI/UserControls/StatisticsUserControl.xaml.cs)
