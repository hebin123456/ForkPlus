		...
		private void ConfigureForStatus()
		{
			...
			switch (_viewModel.Status.Value)
			{
				case ForkSyncStatus.SafeToPush:
					StatusIcon.Source = new Bitmap(AssetLoader.Open(SuccessIcon));
				...
				case ForkSyncStatus.ShouldSyncNoConflict:
					StatusIcon.Source = new Bitmap(AssetLoader.Open(WarningIcon));
				...
				case ForkSyncStatus.MustSyncWithConflict:
					StatusIcon.Source = new Bitmap(AssetLoader.Open(ErrorIcon));
