...
{
	/// <summary>
	/// <see cref="GitHubEnterpriseLoginWindow"/> 的 ViewModel（阶段 3 横向复用）。
	/// 承接 server URL 规范化（TrimEnd）+ 绝对 URI 校验 + token 非空校验。零 WPF using。
	/// </summary>
	/// <remarks>
	/// 复用 BitbucketServerLoginWindowViewModel 模式，差异：ServerUrl 用 <c>TrimEnd</c>（仅去尾斜杠）
	/// 而非 <c>Trim</c>（去首尾）。SetStatus/Hint.Enable/Disable 副作用留 View override。
	/// </remarks>
	public class GitHubEnterpriseLoginWindowViewModel : INotifyPropertyChanged
	{
