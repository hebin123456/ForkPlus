...
namespace ForkPlus.UI.Controls.Editor
{
	...
	/// 阶段 4 里程碑 4.7-a：WPF Brush→Avalonia IBrush；brush.Freeze() 移除（Avalonia 画刷默认不可变）；
	/// Application.Current.TryFindResource → Theme.FindResource（复用 4.3-b 迁移后的资源查找门面）。
	/// </summary>
	public static class HighlightingTypeExtensions
	{
		...
		private static readonly IBrush RemoveBrush = new SolidColorBrush(Color.FromRgb(byte.MaxValue, 229, 229));
		...
		private static readonly IBrush MergeUnresolvedBrush = new SolidColorBrush(Color.FromRgb(byte.MaxValue, 196, 196));
		...
		public static IBrush GetHighlightBrush(this HighlightingType highlightingType, ThemeType theme)
		{
			...
			// 不 Freeze——Avalonia 画刷默认不可变；每次重绘会再调本方法拿到最新画刷。
