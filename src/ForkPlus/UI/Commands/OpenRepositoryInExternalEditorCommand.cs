	...
	public class OpenRepositoryInExternalEditorCommand : IUICommand, IForkPlusCommand
	{
		...
		private void ExecuteInternal(string path, ExternalRepositoryEditor editor)
		{
			...
			if (!File.Exists(text))
			{
				...
				new ErrorWindow(ServiceLocator.Localization.FormatCurrent("Cannot find {0} at '{1}'", editor.Name, text)).ShowDialog();
				return;
			}
			...
			catch (Exception ex)
			{
				...
				new ErrorWindow(ServiceLocator.Localization.FormatCurrent("Cannot run '{0} {1}'.\n{2}", text, path, ex.ToString())).ShowDialog();
			}
		}
