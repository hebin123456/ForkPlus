...
namespace ForkPlus
{
	...
	private static string ResolveGitMmPath()
		...
		/// <summary>
		/// PATH 查找 git-ai.exe 的缓存（与 git-mm 同模式）。
		/// </summary>
		...
		/// 优先使用用户在偏好设置中指定的路径；否则在 PATH 环境变量中查找 <c>git-ai.exe</c>；
		/// 再否则在 git.exe 同目录查找。三者都找不到返回 null（AI 归属功能自动降级隐藏）。
		/// </summary>
		public static string GitAiPath => ResolveGitAiPath();

		/// <summary>
		/// 仅从 PATH 查找的 git-ai.exe 路径（带缓存）。供偏好设置 UI 列出候选时使用。
		/// </summary>
		public static string GitAiPathFromPath
		{
			get
			{
				...
				{
					_cachedGitAiFromPath = FindExecutableInPath("git-ai.exe");
