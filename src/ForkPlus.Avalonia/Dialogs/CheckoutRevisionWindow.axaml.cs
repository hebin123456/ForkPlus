...
{
    // Phase 4.x：Avalonia 版 CheckoutRevisionWindow（真实迁移版，对照 WPF CheckoutRevisionWindow.xaml.cs 280 行）。
    //
    // 对照 WPF 工程 src/ForkPlus/UI/Dialogs/CheckoutRevisionWindow.xaml.cs：
    //   - public partial class CheckoutRevisionWindow : ForkPlusDialogWindow
    //   - 字段: RepositoryUserControl _repositoryUserControl / IGitPoint _gitPoint / Sha _gitPointSha
    //   - 构造函数 (RepositoryUserControl, IGitPoint, Sha)
    //   - GetCommandPreview: "git checkout [--force] {sha}" + dirty 时 StashAndReapply 选中前置 "git stash\n"
    //   - OnSubmit: StashAndReapply 选择 → PerformCheckout
    //     (可选 SaveStashGitCommand + CheckoutRevisionGitCommand + 可选 ApplyStashGitCommand + UpdateSubmodulesGitCommand)
    ...
    //      + SubmodulesToUpdate? submodulesToUpdate + Action<GitCommandResult>? onCompleted 回调
    //   3. GitPointView 自定义控件 → spike 版用 TextBlock 显示 FriendlyName 简化
    ...
    public partial class CheckoutRevisionWindow : global::ForkPlus.Avalonia.Dialogs.ForkPlusDialogWindow
    {
            ...
            Action<GitCommandResult>? onCompleted = null)
        {
            ...
            DialogTitle = Translate("Checkout Commit");
            DialogDescription = Translate("Checkout a particular revision. Repository will be in detached HEAD state.");
            SubmitButtonTitle = Translate("Checkout Commit");
            CancelButtonTitle = Translate("Cancel");
            Title = Translate("Checkout Commit");

            // 对照 WPF: RevisionGitPointView.Value = gitPoint;
            // Avalonia spike: 用 TextBlock 显示 FriendlyName 简化
            RevisionGitPointTextBlock.Text = gitPoint?.FriendlyName ?? gitPointSha.ToAbbreviatedString();
        ...
        protected override void OnSubmit()
        {
            ...
            // 对照 WPF: _repositoryUserControl.JobQueue.Add(FormatTranslate("Checkout '{0}'", gitPoint.FriendlyName), ...)
            // Avalonia: Task.Run + Dispatcher.UIThread.Post + JobMonitor（spike 版不做 undo）
            Task.Run(delegate
            {
                JobMonitor monitor = new JobMonitor();
                GitCommandResult result = PerformCheckout(
                    gitModule, gitPoint, gitPointSha,
                ...
                if (monitor.IsCanceled)
                {
                    ...
                    {
                        try { _onCompleted?.Invoke(GitCommandResult.Success()); } catch (Exception ex) { Log.Error("CheckoutRevisionWindow onCompleted callback failed", ex); }
                ...
                Dispatcher.UIThread.Post(delegate
                {
                    try { _onCompleted?.Invoke(result); } catch (Exception ex) { Log.Error("CheckoutRevisionWindow onCompleted callback failed", ex); }
                    Close(result);
                });
            });
        }
            ...
            JobMonitor monitor)
        {
            ...
            if (stashAndReapply == StashAndReapply.Required)
            {
                ...
                GitCommandResult<bool> stashResult = new SaveStashGitCommand().Execute(
                    gitModule,
                    $"Autostash. Switch from '{sourceString}' to '{gitPoint.FriendlyName}' (detached HEAD) {DateTime.Now}",
                    false,
                    monitor);
            ...
            GitCommandResult checkoutResult = new CheckoutRevisionGitCommand().Execute(gitModule, gitPointSha, monitor, discardLocalChanges);
            if (monitor.IsCanceled)
            {
                return GitCommandResult.Failure(new GitCommandError.Cancelled());
            }
