...
{
    // Phase 4.27b：Avalonia 版 BitbucketServerLoginWindow（真实迁移版，对照 WPF BitbucketServerLoginWindow.xaml.cs 113 行）。
    //
    // 对照 WPF 工程 src/ForkPlus/UI/Dialogs/Accounts/BitbucketServerLoginWindow.xaml.cs：
    //   - public partial class BitbucketServerLoginWindow : ForkPlusDialogWindow, IServiceLoginWindow
    //   - 字段: JobQueue / Account / ServerTextBox + TokenTextBox（双字段）
    //   - IsSubmitAllowed override: Uri.TryCreate(ServerUrl) + !IsNullOrEmpty(Token)
    //   - OnSubmit: 构造 PrivateAccessTokenAuthentication + Connection + BitbucketServerService
    //     → service.GetUser() → 成功则 LogOut 旧 + 创建新 Account + AddOrUpdate
    //   - OpenPersonalAccessTokenConfigurationUrlButton_Click: 打开 ServerUrl + "/plugins/servlet/access-tokens/add"
    //
    // Avalonia 版差异：
    ...
    //
    // 注：本类结构与 GiteaLoginWindow 几乎完全一致（仅 RemoteType / Service 类不同）
    public partial class BitbucketServerLoginWindow : global::ForkPlus.Avalonia.Dialogs.ForkPlusDialogWindow, IServiceLoginWindow
    {
        ...
        public BitbucketServerLoginWindow(Account? account = null, Action? onAccountChanged = null)
        {
            ShowFooter = true;
            InitializeComponent();
            SetFooter(Footer);
                ...
                else
                {
                    Dispatcher.UIThread.Post(delegate
                    {
                        ...
                        else
                        {
                            ...
                            {
                                Log.Error("BitbucketServerLoginWindow onAccountChanged callback failed", ex);
                            }
                            CloseWithOk();
                        }
                    });
