...
{
    // Phase 4.23b：Avalonia 版 GiteaLoginWindow（真实迁移版，对照 WPF GiteaLoginWindow.xaml.cs 122 行）。
    //
    // 对照 WPF 工程 src/ForkPlus/UI/Dialogs/Accounts/GiteaLoginWindow.xaml.cs：
    //   - public partial class GiteaLoginWindow : ForkPlusDialogWindow, IServiceLoginWindow
    //   - 字段: JobQueue _jobQueue / Account Account
    //   - 构造函数 (Account account = null):
    //     * ShowLogo=false / ShowHeader=false
    //     * SubmitButtonTitle = "Sign In"
    //     * ServerTextBox.Text = account?.ServerUrl
    //   - IsSubmitAllowed override: Uri.TryCreate(ServerUrl) + !IsNullOrEmpty(TokenTextBox.Text)
    //   - OnSubmit:
    //     * 构造 PrivateAccessTokenAuthentication / Connection / GiteaService
    //     * service.GetUser() → 成功则创建 Account + AccountManager.AddOrUpdate
    //     * MainWindow.ActiveRepositoryUserControl?.InvalidateAndRefresh(SubDomain.Remotes)
    //   - OpenAccessTokenConfigurationUrlButton_Click: new Uri(ServerUrl + "/user/settings/applications").OpenInBrowser()
    //
    // Avalonia 版差异：
    ...
    //   5. DisableEditableControls/EnableEditableControls → 手动禁用/启用 ServerTextBox + TokenTextBox
    //   6. MainWindow.ActiveRepositoryUserControl?.InvalidateAndRefresh(SubDomain.Remotes)
    //      → 注入 Action? onAccountChanged 回调（解耦，spike 版 RepositoryUserControl 未迁移）
    //   7. IServiceLoginWindow 接口已在 Core（Phase 4.21b），namespace ForkPlus.Accounts
    //   8. TextChanged 事件签名：EventArgs → TextChangedEventArgs
    public partial class GiteaLoginWindow : global::ForkPlus.Avalonia.Dialogs.ForkPlusDialogWindow, IServiceLoginWindow
    {
        ...
        // 构造函数签名与 WPF 相同 + 新增 Action? onAccountChanged 回调（解耦 MainWindow 依赖）
        public GiteaLoginWindow(Account? account = null, Action? onAccountChanged = null)
        {
            ShowFooter = true;
            InitializeComponent();
            SetFooter(Footer);
            ...
            // 对照 WPF: ShowLogo=false / ShowHeader=false / SubmitButtonTitle = "Sign In"
            ...
            // 对照 WPF: ServerTextBox.TextChanged += UpdateSubmitButton;
                    ...
                    Dispatcher.UIThread.Post(delegate
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    Log.Error("GiteaLoginWindow onAccountChanged callback failed", ex);
                                }
                                CloseWithOk();
                            }
                        }
        ...
        // 对照 WPF: OpenAccessTokenConfigurationUrlButton_Click
        public void OpenAccessTokenConfigurationUrlButton_Click(object? sender, global::Avalonia.Interactivity.RoutedEventArgs e)
        {
