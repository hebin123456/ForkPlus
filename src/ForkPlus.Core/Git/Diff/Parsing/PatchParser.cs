...
{
    /// <summary>
    /// Cross-platform base patch parser. Subclasses provide the tokenizer
    /// implementation through <see cref="ReadTokens"/>; on platforms where the
    /// Biturbo FFI is available the subclass is
    /// <c>ForkPlus.Git.Diff.Parsing.BiturboPatchParser</c>.
    /// </summary>
    public class PatchParser
    {
        public virtual GitCommandResult<Patch> Parse(string input, string srcPrefix = "forkSrcPrefix/", string dstPrefix = "forkDstPrefix/")
        {
        ...
        /// <summary>
        /// Tokenize a UTF-8 diff using platform-specific backends.
        /// </summary>
        protected virtual GitCommandResult<Token[]> ReadTokens(byte[] diffUtf8, byte[] srcPrefixUtf8, byte[] dstPrefixUtf8)
        {
            // Pure managed fallback that returns a single Other token covering
            // the entire input. Cross-platform callers that need real diff
            // parsing must supply a tokenizer through this method.
            return GitCommandResult<Token[]>.Success(new Token[]
            {
                new Token(TokenType.Other, new Range(0, diffUtf8.Length))
            });
        }
