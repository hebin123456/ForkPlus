	...
	public class DiscardFileChangesGitCommand
	{
		public GitCommandResult Execute(GitModule gitModule, ChangedFile[] files, JobMonitor monitor)
		{
			...
			if (list2.Count > 0)
			{
				...
				GitCommand command = new GitCommand(ServiceLocator.GitEnvironment.OverrideCredentialHelperBt, "checkout-index", "--index", "--force", "--stdin", "-z");
		...
		private GitCommandResult DeleteFile(GitModule gitModule, string filePath)
		{
			...
			try
			{
				...
				{
					GitRequestResult gitRequestResult = default(GitRequest).Path(ServiceLocator.GitEnvironment.BashPath).CurrentDir(gitModule.Path).Command("-c", "rm -f '" + text + "'")
						.ExecuteBt();
