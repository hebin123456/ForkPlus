...
namespace ForkPlus.Git.Commands
{
	public class ResetFilesAtRevisionGitCommand
	{
		public GitCommandResult Execute(GitModule gitModule, ChangedFile[] changedFiles, string targetString, JobMonitor monitor)
		{
			...
			{
				GitCommand command = new GitCommand(ServiceLocator.GitEnvironment.OverrideCredentialHelper, "reset", "-q", targetString, "--", changedFile.Path.Quotify());
				GitRequestResult gitRequestResult = new GitRequest(gitModule).Command(command).Execute(monitor);
				if (!gitRequestResult.Success)
				{
					return GitCommandResult.Failure(gitRequestResult.ToGitCommandError());
				}
				GitCommand command2 = new GitCommand(ServiceLocator.GitEnvironment.OverrideCredentialHelper, "checkout", targetString, "--", changedFile.Path.Quotify());
