...
namespace ForkPlus.Git.Commands
{
	public class BenchmarkGitCommand
	{
		...
		public GitCommandResult<BenchmarkResult> Execute(GitModule gitModule, JobMonitor monitor, [Null] Action<BenchmarkResult> callback)
		{
			...
			int minPagesCount = ServiceLocator.UserSettings.MinPagesCount;
			...
			GitCommandResult<double> gitCommandResult7 = Run("Reading commits bt", context, () => new GetRevisionStorageGitCommand().Execute(gitModule, referenceStorage, ServiceLocator.UserSettings.RevisionSortOrder == RevisionSortOrder.Topo, reflog: false, 0, minPagesCount, requiredShas, DateTime.UtcNow.MillisecondsSince1970(), commitGraphCache, monitor).ToGitCommandResult());
		...
		{
			return ServiceLocator.Localization.Translate(text, ServiceLocator.UserSettings.UiLanguage);
		}
		...
		private static GitCommandResult GetGitLog(GitModule gitModule, RevisionSortOrder sortOrder, bool reflog, JobMonitor cancellationToken)
		{
			...
			gitCommand.Add("--tags");
			gitCommand.Add($"--max-count={ServiceLocator.UserSettings.MaxCommitCount}");
