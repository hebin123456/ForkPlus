...
namespace ForkPlus.Git.Commands
{
	public class ContinueCherryPickGitCommand
	{
		...
		{
			GitCommand command = new GitCommand(ServiceLocator.GitEnvironment.OverrideCredentialHelper, "cherry-pick", "--continue");
			...
			if (GitCommandError.CherryPickNothingToCommit.Match(gitRequestResult))
			{
				...
				do
				{
					GitCommand command2 = new GitCommand(ServiceLocator.GitEnvironment.OverrideCredentialHelper, "cherry-pick", "--skip");
