		...
		public GitCommandResult Execute(GitModule gitModule, UndoEntry target, JobMonitor monitor)
		{
			...
			if (!string.IsNullOrEmpty(target.CurrentBranchName))
			{
				...
				if (currentBranch != target.CurrentBranchName)
				{
					...
					GitCommand checkoutCmd = new GitCommand(ServiceLocator.GitEnvironment.OverrideCredentialHelper, "checkout", target.CurrentBranchName);
			...
			if (!string.IsNullOrEmpty(target.HeadSha))
			{
				...
				if (currentHead != target.HeadSha)
				{
					...
					GitCommand resetCmd = new GitCommand(ServiceLocator.GitEnvironment.OverrideCredentialHelperBt, "reset", "--hard", target.HeadSha);
			...
			if (!string.IsNullOrEmpty(target.PreOperationStashSha))
			{
				...
				GitCommand stashApplyCmd = new GitCommand(ServiceLocator.GitEnvironment.OverrideCredentialHelper, "stash", "apply", "--index", target.PreOperationStashSha);
