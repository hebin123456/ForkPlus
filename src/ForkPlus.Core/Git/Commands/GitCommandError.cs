...
namespace ForkPlus.Git.Commands
{
	public abstract class GitCommandError
	{
		...
		public class ParseError : GitCommandError
		{
			...
			public override string FriendlyDescription => ServiceLocator.Localization.Translate(ErrorMessage, ServiceLocator.UserSettings.UiLanguage);
		...
		public class GitError : GitCommandError
		{
			...
			private static string Translate(string output)
			{
			 ...
			 {
			  return string.Format(ServiceLocator.Localization.Translate("fatal: There is no path '{0}' in the commit", ServiceLocator.UserSettings.UiLanguage), match.Groups[1].Value);
			 });
			 output = PermissionDeniedOpenRegex.Replace(output, delegate(Match match)
			 {
			  return string.Format(ServiceLocator.Localization.Translate("error: open('{0}'): Permission denied", ServiceLocator.UserSettings.UiLanguage), match.Groups[1].Value);
			 });
			 output = CannotHashRegex.Replace(output, delegate(Match match)
			 {
			  ...
			  if (key.Contains("{0}"))
			  {
			   return string.Format(ServiceLocator.Localization.Translate(key, ServiceLocator.UserSettings.UiLanguage), match.Groups[1].Value);
			  }
			  return ServiceLocator.Localization.Translate(key, ServiceLocator.UserSettings.UiLanguage);
			 });
			 return output;
			}
		}
