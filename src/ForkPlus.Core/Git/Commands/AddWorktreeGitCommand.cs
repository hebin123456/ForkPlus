...
namespace ForkPlus.Git.Commands
{
	public class AddWorktreeGitCommand
	{
		...
		{
			GitCommand command = new GitCommand(ServiceLocator.GitEnvironment.OverrideCredentialHelperBt, "worktree", "add", path, branch);
		...
		{
			GitCommand command = new GitCommand(ServiceLocator.GitEnvironment.OverrideCredentialHelperBt, "worktree", "add", "-b", branch, path, startSha.ToString());
			GitRequestResult gitRequestResult = new GitRequest(gitModule).Command(command).ExecuteBt(monitor);
			if (!gitRequestResult.Success)
			{
