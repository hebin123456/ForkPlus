...
namespace ForkPlus.Accounts.AiServices
{
	internal class OpenAiService : RestClientBase
	{
		...
		{
			return !string.IsNullOrWhiteSpace(ServiceLocator.UserSettings.AiReviewServiceUrl)
				&& !string.IsNullOrWhiteSpace(ServiceLocator.UserSettings.AiReviewApiKey)
				&& !string.IsNullOrWhiteSpace(ServiceLocator.UserSettings.AiReviewSelectedModel);
		}

		public static OpenAiService CreateFromAiReviewSettings()
		{
			string serviceUrl = NormalizeServiceUrl(ServiceLocator.UserSettings.AiReviewServiceUrl);
			PrivateAccessTokenAuthentication authentication = new PrivateAccessTokenAuthentication(serviceUrl, "ai-review", ServiceLocator.UserSettings.AiReviewApiKey);
			return new OpenAiService(new Connection(serviceUrl, authentication, ServiceLocator.UserSettings.AiReviewTimeoutSeconds), ServiceLocator.UserSettings.AiReviewSelectedModel);
		}
		...
		public ServiceResult<OpenAiResponse> GenerateCommitMessage(string patchString, GitModule gitModule, JobMonitor monitor, Action<string> onChunk = null)
		{
			...
			int pageGuideLinePosition = ServiceLocator.UserSettings.PageGuideLinePosition;
			int commitSubjectLowLimit = ServiceLocator.UserSettings.CommitSubjectLowLimit;
			int commitSubjectHighLimit = ServiceLocator.UserSettings.CommitSubjectHighLimit;
			...
			{
				commitMessageRegex = ServiceLocator.UserSettings.CommitMessageRegex;
			}
		...
		{
			string language = ServiceLocator.UserSettings.UiLanguage;
		...
		{
			int retryCount = Math.Max(0, ServiceLocator.UserSettings.AiReviewRetryCount);
		...
		private static int MaxQueuedWaitSeconds()
		{
			...
			return Math.Max(ServiceLocator.UserSettings.AiReviewTimeoutSeconds, 1800);
		}
