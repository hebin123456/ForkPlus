...
{
    // Phase 2.1：ThemeType 的纯逻辑扩展方法。
    // 从 WPF 工程 src/ForkPlus/UI/ThemeTypeExtensions.cs 拆出，只含无 UI 依赖的方法。
    // WPF / Avalonia 工程都能用，避免逻辑重复。
    //
    // 原 WPF ThemeTypeExtensions.ResourceUri() 返回 pack:// URI（WPF-specific），
    // 留在 WPF 工程；Avalonia 工程有自己的 ResourceUri 扩展（avares:// URI）。
