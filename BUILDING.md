...
| Git | ≥ 2.44（推荐） | 应用内 git 操作 + 测试 | 低于推荐版本启动时弹警告，部分功能异常（变基预检 `git replay` 需 2.44+、交互式变基 `--update-refs` 需 2.38+）；应用优先使用内置 git 实例（2.50.1），缺失时回退系统 git。**跑测试必须 ≥ 2.44**，否则 E2E 变基模块用例失败 |
...
- 全量 4400+ 用例，含单元测试与 Avalonia.Headless UI 端到端测试（Skia 软件渲染，无需显示服务器，Linux 服务器上可直接跑）；
- E2E 套件会执行真实 git 管线（含 GitFlow / LFS 链路），需要上表的 gitflow-avh 与 git-lfs；
- 失败时的证据截图落在 `docs/evidence/`（`.gitignore` 已忽略顶层过程截图）；
- CI 在 tag 构建与手动触发时于 ubuntu runner 上运行全量测试，本地与 CI 环境差异见 [`.github/workflows/build.yml`](.github/workflows/build.yml) 注释。
