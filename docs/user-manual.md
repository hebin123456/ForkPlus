...
> 适用版本：ForkPlus v4.1+（跨平台版） · 文档日期：2026-09-13
>
> 本手册基于全量 UI 自动化测试用例（26 个模块、163 个用例、213 张截图证据）整理而成，
> 覆盖 ForkPlus 的全部用户可见功能。文中插图均取自测试套件的真实渲染截图：
> 主界面统一为 1920×1080，对话框按其真实比例呈现。
>
> 本次更新（v4.1.0 / v4.1.1）：补全[自动更新](#更新检查)全链路、下载进度条与随时取消、
> 首次启动"更新内容"弹窗，以及"检查更新"窗口内新增的[重置此版本](#更新检查)就地修复入口。
...
ForkPlus 从 v4.0 起为跨平台版本，UI 层从 Windows 专用的 WPF 技术栈迁移至
[Avalonia 12](https://avaloniaui.net/) 框架，可在 **Windows、Linux、macOS** 三大桌面平台运行。
功能与 WPF 原版（v3.x）对齐，并针对跨平台环境做了适配修复（如 Unix 下的 SSH 密钥工具路径、
...
详细的构建环境说明（SDK 版本、三方件自动拉取机制等）见仓库根目录 `BUILDING.md`。
...
菜单 **帮助 → 检查更新（Help → Check for Updates）** 打开"检查更新"窗口，打开即自动检测：
检测中显示进度，完成后进入结果面板，包含**有更新 / 已是最新 / 网络失败**三种终态。

![更新检查结果](evidence/e2e/25-common-dialogs/09-updatecheck-result.png)

检测到新版本时，窗口内嵌当前版本的更新内容，并提供**下载**按钮，具备以下自动更新能力：

- **全自动升级（v4.1.0）**：点击下载后不再跳转浏览器手动安装，而是自动从 GitHub 获取当前平台的
  安装包（由后台 `ForkPlus.AutoUpdater` 子进程完成）。整个流程为：**下载 → 解压 → 关闭应用 →
  备份并替换安装目录 → 自动重启**回到新版本，全程无需离开应用。
- **下载进度条 + 随时取消（v4.1.0）**：下载期间展示实时进度（已收/总字节数、百分比、瞬时速度），
  任意时刻可点**取消**（此时取消按钮语义变为"取消下载"）中止——安装目录尚未被触碰，可重试或关闭窗口。
  解压及之后的替换阶段不可安全中断，取消按钮会自动收起。
- **失败回滚（v4.1.0）**：替换失败时自动回滚（备份安装目录原样移回），本地安装不留半新半旧状态；
  失败原因显示在窗口底部状态区。
- **首次启动"更新内容"弹窗（v4.1.0）**：升级到新版本后首次启动，会自动弹出该版本的更新内容
  （更新内容弹窗，界面标题为 What's New），数据来自随包分发的 `Docs/RELEASE_NOTE.md`；
  同版本后续启动不再弹出。

#### 重置此版本（v4.1.1）

若当前版本偶尔**损坏**（升级中断、文件缺失或出错）又暂时没有新版本可用，无需再手动删包重装。
在"检查更新"窗口底部点击**重置此版本**即可就地修复当前版本，分两步确认：

1. **确认重置当前版本**：弹窗提示"当前版本 {0} 将从 GitHub 重新下载并替换本地安装"（含警告图标，
   提供"重置 / 取消"）——确认后按当前版本 + 当前平台构造 GitHub 直链，重新拉取当前版本的安装包。
2. **是否同时重置设置**：二次弹窗提示"重置设置将把所有偏好（仓库、外观、语言等）恢复为默认值，
   且无法撤销"——选择**重置设置**会在替换成功后删除本地设置文件（`settings.json`）；
   选择**保留设置**则照常重置当前版本、设置文件原样保留。

确认后复用 v4.1.0 的自动更新链路完成：**下载 → 解压 → 关闭应用 → 备份替换安装目录 → 自动重启**，
交互与自动更新一致（进度面板 + 可取消下载 + 失败展示在状态区）。任一步取消即中止，不启动重置流；
重置进行中入口隐藏、防止重复触发。更新失败路径下设置必须原样保留，仅替换成功后才会删除设置文件。
...
git ≥ 2.38 对本地路径子模块有安全限制。ForkPlus 已内置规避（自动加
