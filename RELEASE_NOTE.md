...
- **方案选型**：调研后选择集成 [tokei](https://github.com/XAMPPRocky/tokei)（MIT/Apache-2.0 双协议的 Rust 代码统计工具，支持 200+ 语言，区分 code/comments/blanks，尊重 .gitignore，提供稳定的 `--output json`）。相比在 biturbo native 侧加 API 或纯 managed 实现一遍语言识别，复用 tokei 最准确。但 tokei 上游 release 只发布源码、不发布预编译二进制，故采用构建期 `cargo install` 从源码编译的方案—— [ForkPlus.Assets.Legal.txt](file:///workspace/src/ForkPlus/ForkPlus.Assets.Legal.txt)
- **构建期编译 tokei.exe**：在 `ForkPlus.csproj` / `ForkPlus.Tests.csproj` 新增 `RestoreTokei` target（`BeforeTargets=Build`，`third_party/tokei.exe` 缺失时用 PowerShell 调 `cargo install tokei --version 14.0.0 --locked --root <临时目录> --no-track` 从源码编译，再把 `bin/tokei.exe` 移到 `third_party/tokei.exe`；cargo 不在 PATH 时报清晰错误提示装 Rust 或手动放置 tokei.exe），并在 `CopyHelperExecutables` 里随构建拷贝到输出目录。CI 用 `actions/cache` 缓存 `third_party/tokei.exe`（key=`tokei-14.0.0-windows-x86_64`），命中后跳过编译秒过；windows-latest 自带 Rust 1.96 工具链。`third_party/tokei.exe` 和 `third_party/.tokei-install/` 加入 `.gitignore` 不提交—— [ForkPlus.csproj](file:///workspace/src/ForkPlus/ForkPlus.csproj)、[build-windows.yml](file:///workspace/.github/workflows/build-windows.yml)、[.gitignore](file:///workspace/.gitignore)
- **数据获取命令**：新建 `GetCodeLineStatsGitCommand`，spawn tokei.exe 跑 `--output json` 并解析。两种模式：
  - **snapshot 模式**（refSpec 为空）：直接在工作区目录跑 tokei，统计当前文件
  - **历史 ref 模式**（refSpec 非空，分支/tag/sha）：`git archive --format=zip -o <temp> <ref>` 导出，再用 .NET `ZipFile.ExtractToDirectory` 解压到临时目录跑 tokei，完成后清理，避免污染工作区
...
- **修复：代码行数全是 0**：tokei v14 的 JSON 把 `code/comments/blanks` 平铺在语言对象下、`lines` 是方法不序列化、`files` 由 `reports` 数组长度决定；旧代码按 v13 的 `lang.lines.code` 嵌套取值导致全 0。`FromTokeiJson` 改成 v13/v14 兼容解析——平铺优先、`lines` 为对象时退化嵌套、`files` 优先取 `reports` 数组长度。已用 tokei v14.0.0 实测 JSON 格式验证（C# 仓库 134129 行 code 正确解析）—— [CodeLineStats.cs](file:///workspace/src/ForkPlus/Git/Commands/CodeLineStats.cs)
- **增强：AI 辅助开发按钮加图标 + dark 适配**：工具栏"AI-Assisted Development"按钮此前用 Unicode 字符 `✦`（TextBlock）当图标，与旁边控制台按钮（`ConsoleIcon` 图像）风格不一致且无 dark 适配。新增 sparkle（✦ 四角星）PNG 图标 `assets/ai.png`（light 版深灰）+ `assets/aidark.png`（dark 版浅灰），延续原字符设计语言；在 `Images.Light.xaml` / `Images.Dark.xaml` 注册 `AiIcon`（复用现有双字典整体替换机制，主题切换自动跟随）；工具栏按钮的 `✦` TextBlock 换成 `<Image Source="{DynamicResource AiIcon}"/>`，与控制台按钮布局一致—— [ToolbarUserControl.xaml](file:///workspace/src/ForkPlus/UI/UserControls/ToolbarUserControl.xaml)、[Images.Light.xaml](file:///workspace/src/ForkPlus/Theme/Styles/Images.Light.xaml)、[Images.Dark.xaml](file:///workspace/src/ForkPlus/Theme/Styles/Images.Dark.xaml)
- **增强：Ref 下拉改为置顶搜索的 Popup**：分支/tag 多时下拉难以查找。改为 Button（显示当前 ref）+ Popup：Popup 内顶部固定一行 `PlaceholderTextBox`（复用侧边栏搜索框同款 `SearchPanelPlaceholderTextBox` 样式 + `SearchOnIcon` 图标），下方是可独立滚动的 ListBox；搜索框在 Grid.Row=0，列表在 Grid.Row=1，上下滚动列表不影响搜索框位置。打开 Popup 时清空搜索 + 聚焦搜索框 + 全选，关闭时清空搜索文本。Workspace 项始终保留在过滤结果中—— [StatisticsUserControl.xaml](file:///workspace/src/ForkPlus/UI/UserControls/StatisticsUserControl.xaml)、[StatisticsUserControl.xaml.cs](file:///workspace/src/ForkPlus/UI/UserControls/StatisticsUserControl.xaml.cs)
- **新增：分支右键菜单"代码统计"入口**：本地分支右键菜单（"Copy Branch Name"前）新增"Code statistics..."菜单项，点击以该分支名作为初始 refSpec 打开 `RepositoryStatisticsWindow`，自动选中该分支并触发 tokei 统计，同时滚动到代码行数统计区域（Row 6，`CodeLinesSection.BringIntoView()`，延迟到 Loaded 优先级确保布局完成）。`ShowStatistics` / `RepositoryStatisticsWindow` / `ShowRepositoryStatisticsWindowCommand` 各加重载接受 `initialRef` + `scrollToCodeLines` 参数。i18n key `Code statistics...` 同步到 7 语言—— [SidebarUserControl.xaml.cs](file:///workspace/src/ForkPlus/UI/UserControls/SidebarUserControl.xaml.cs)、[StatisticsUserControl.xaml.cs](file:///workspace/src/ForkPlus/UI/UserControls/StatisticsUserControl.xaml.cs)、[RepositoryStatisticsWindow.xaml.cs](file:///workspace/src/ForkPlus/UI/Dialogs/RepositoryStatisticsWindow.xaml.cs)、[ShowRepositoryStatisticsWindowCommand.cs](file:///workspace/src/ForkPlus/UI/Commands/ShowRepositoryStatisticsWindowCommand.cs)
- **修复：代码统计窗口不滚动到代码行数区域**：原实现在 ShowStatistics 同步阶段用 `DispatcherPriority.Loaded` 调 `BringIntoView`，但此时 `UpdatePreview` 的异步统计任务还没完成，`StatsContainer` 仍处于 `Collapsed` 状态，`CodeLinesSection` 实际高度为 0，滚动无效。改为用 `_pendingScrollToCodeLines` 标志记录待滚动请求，在 `UpdatePreview` 异步回调里 `StatsContainer.Show()` + `UpdatePlots()` 完成后，以 `DispatcherPriority.Render` 延迟执行 `BringIntoView`，确保布局测量完成—— [StatisticsUserControl.xaml.cs](file:///workspace/src/ForkPlus/UI/UserControls/StatisticsUserControl.xaml.cs)
- **优化：代码统计 git archive 失败的错误信息**：原实现 `git archive` 失败时只返回 `"git archive failed for ref: <ref>"`，丢弃 git 的 stderr，用户无法判断真实原因（ref 不存在 / worktree 异常 / archive 其他错误）。改为：失败时先调 `git rev-parse --verify <ref>^{commit}` 验证 ref 是否可解析，区分"ref 不存在"和"archive 其他错误"，并把 git stderr 一并拼进错误信息；tar 解压失败也输出具体原因（超时 / exit code / tar.exe 不可用）—— [GetCodeLineStatsGitCommand.cs](file:///workspace/src/ForkPlus/Git/Commands/GetCodeLineStatsGitCommand.cs)
- **修复：选择当前工作区分支报 git archive failed**：根因——`git archive <当前检出的分支>` 会因 worktree 占用而失败（git archive 不能 archive 当前 HEAD 检出的 ref）。新增 `IsCurrentWorktreeRef` 判断：用 `git symbolic-ref --quiet --short HEAD` 取当前分支名比较，再用 `git rev-parse HEAD` 与 `git rev-parse <refSpec>` 比较完整 sha（覆盖 detached HEAD + 短 sha / refs/heads/x 等输入），命中任一即走 workspace snapshot 模式（直接在工作区目录跑 tokei），不再调 git archive。这样选当前分支时直接统计工作区文件，既避免报错也更快—— [GetCodeLineStatsGitCommand.cs](file:///workspace/src/ForkPlus/Git/Commands/GetCodeLineStatsGitCommand.cs)
- **修复：tar 解压报 "Invalid empty pathname: Unknown error"**：根因——历史 ref 模式原用 `git archive --format=tar` 生成 tar 再调系统 `tar.exe`（Windows 10 1803+ 自带的 bsdtar/libarchive）解压，但 bsdtar 对 git archive 生成的 tar 中的 `pax_global_header` 等扩展条目报 "Invalid empty pathname: Unknown error" 导致解压失败。改为 `git archive --format=zip` 生成 zip + .NET 原生 `System.IO.Compression.ZipFile.ExtractToDirectory` 解压（`ForkPlus.csproj` 加 `System.IO.Compression` / `System.IO.Compression.FileSystem` 引用），彻底不依赖系统 tar.exe—— [GetCodeLineStatsGitCommand.cs](file:///workspace/src/ForkPlus/Git/Commands/GetCodeLineStatsGitCommand.cs)、[ForkPlus.csproj](file:///workspace/src/ForkPlus/ForkPlus.csproj)
- **修复：代码统计中文分支名乱码**：根因——git 默认 `core.quotePath=true` 会对输出中的非 ASCII 字符做八进制转义（如 `\344\270\255` 代替 `中`），导致 `git for-each-ref` 列出的中文分支名、`git symbolic-ref` 输出的当前分支名、错误信息中的 ref 名都是乱码。所有相关 git 调用（`for-each-ref` / `symbolic-ref` / `rev-parse` / `archive`）统一加 `-c core.quotePath=false` 让 git 输出原始 UTF-8 字节（`GitRequest` 的 `StandardOutputEncoding` / `StandardErrorEncoding` 已设为 UTF8，配合 quotePath=false 即可正确显示）—— [GetCodeLineStatsGitCommand.cs](file:///workspace/src/ForkPlus/Git/Commands/GetCodeLineStatsGitCommand.cs)、[StatisticsUserControl.xaml.cs](file:///workspace/src/ForkPlus/UI/UserControls/StatisticsUserControl.xaml.cs)
- **修复：zip 解压报 "未能找到路径...的一部分"（长路径）**：根因——.NET Framework 4.7.2 的 `ZipFile.ExtractToDirectory` 内部用 `FileStream`，解压路径超过 MAX_PATH(260) 时 Win32 返回 `ERROR_PATH_NOT_FOUND`，.NET 映射成 `DirectoryNotFoundException`（"未能找到路径...的一部分"）。仓库内深嵌套文件（如 node_modules、src 多层目录）解压到临时目录时路径极易超 260。改为逐条目手动解压 `ZipArchive` + `ExtractToFile`，对超过 248 字符的路径加 `\\?\` 前缀（Win32 长路径前缀，支持到 ~32767 字符）；临时目录名也从 `ForkPlus_tokei_<32位guid>` 缩短为 `fpt_<12位guid>` 降低基路径长度；`TryDeleteDirectory` 清理时同样加 `\\?\` 前缀以删除含长路径文件的临时目录—— [GetCodeLineStatsGitCommand.cs](file:///workspace/src/ForkPlus/Git/Commands/GetCodeLineStatsGitCommand.cs)
