...
- **用户手册（docs/manual）功能全覆盖**：在既有 25 章手册基础上，补写了此前未覆盖的功能章节——二分查找（Bisect，好/坏/跳过/中止在通知条操作）、快速启动窗口（Quick Launch，Ctrl+P）、文件树导出与跨版本外部比对（右键选中两个修订 → "Compare File Trees in {工具}"）、Git Mm 上传弹窗的"允许空提交"（`--honor-no-changes`）、Git Mm 子仓库标签页拖拽重排。
- **删除远端弹窗命令预览（RemoveRemoteWindow）**：删除远端改为专用弹窗，展示将要执行的 `git remote remove <name>` 命令预览；通知条/侧边栏触发的删除远端一律经过该弹窗，含 8 语言翻译。

### 变更

- **远程弹窗命令预览排查收口**：逐一核对添加/编辑/删除远端、变更远程跟踪、跟踪远程分支等全部远程相关弹窗的 Git 命令预览可见性，缺失者补全。

### 平台覆盖

| 平台 | RID | 产物 |
|------|-----|------|
| Windows x64 | `win-x64` | `ForkPlus-4.1.6-windows-x64.zip` |
| Linux x64 | `linux-x64` | `ForkPlus-4.1.6-linux-x64.zip` |
| Linux ARM64 | `linux-arm64` | `ForkPlus-4.1.6-linux-arm64.zip` |
| macOS ARM64 | `osx-arm64` | `ForkPlus-4.1.6-macos-arm64.zip` |
...
- **"检查远端同步状态"/"跟踪"子菜单：点一下搜索框，右键菜单直接消失**：搜索框行的 MenuItem 是"叶子"（Items.Count==0）且 StaysOpenOnClick=true，但 WPF→Avalonia 兼容层（MenuExtensions）的三个叶子点击关闭入口（AttachCloseOnLeafClick 闭包、MenuItem_CloseOwningMenuOnClick、ContextMenu_PointerReleasedCloseLeafItem）此前不看 StaysOpenOnClick，一律关整个 ContextMenu——而 Avalonia 的 DefaultMenuInteractionHandler.PointerReleased 会把 MenuItem.Header 里的 TextBox 点击当叶子项点击 RaiseClick（WPF 里 TextBox 吞掉鼠标事件不会走到这）。三处关闭入口统一补上 !StaysOpenOnClick 判断（对齐 Avalonia 原生 Click(item) 语义），搜索框行/禁用状态行等"点了不该关"的叶子保持菜单打开。新增守卫测试 `SearchableSubmenuInteractionTests`（生产同构装配：SetItems 生产入口 + AttachSubmenuDragCaptureGuard 生产入口 + SubmenuOpened 聚焦钩子）：点击搜索框菜单保持打开、拖拽子菜单滚动条全程滚动生效、输入过滤匹配项可见、焦点被抢后守卫归还搜索框。
- **"检查远端同步状态"/"跟踪"子菜单：滚动条拉不动、一点列表框就消失**：Avalonia DefaultMenuInteractionHandler.PointerMoved 内部有个 HACK——按住的指针不在菜单项边界内就 Capture(null) 强制松开捕获；子菜单弹层（PopupRoot）与父菜单行跨坐标系，滚动条 Thumb 拖拽的首个 Move 必被误判"越界"，捕获被掐断 → 滚动条拉不动（headless 实测：按下 captured=Thumb，一次 Move 后 captured=空，滚动停在第一步）；此后指针带键滑出弹层，子菜单还会因 PointerExited 延时自关——用户看到"一点就消失"。修复：新增 MenuExtensions.AttachSubmenuDragCaptureGuard，挂在分组 MenuItem（bubble 路径先于 ContextMenu 层的交互处理器），拖拽进行中（指针已被本子菜单内滚动条 Thumb 或搜索框 TextBox 捕获）时标记 PointerMoved 已处理；Thumb/TextBox 自身的拖拽处理在事件源处先于守卫执行不受影响，未按下时无捕获、悬停高亮等常规菜单行为也不受影响。"检查远端同步状态"与"跟踪"两处分组菜单均走同一工厂（CreateSearchableRemoteGroupMenuItem）一并修复。
- **"检查远端同步状态"/"跟踪"子菜单：弹框宽度偏宽、搜索框与弹框宽度不和谐、行右侧大片留白**：默认叶子行模板为图标列 + 手势列 + 两个 20px 箭头/快捷键槽位预留约 90px 行铬——搜索框只按自身 MinWidth 渲染（不跟随弹层宽度，视觉上明显窄于弹框），且包着搜索框的行把整个弹层反向撑到"搜索框宽 + 90px"，分支行右侧出现大片留白。修复分两步：先收敛搜索框 MinWidth（220 → 160 → 200），再为搜索框行引入专用主题 `SearchableSubmenuSearchRowMenuItem`（实例级 Theme 覆盖隐式 MenuItem 主题，仅影响本行）：去掉全部槽位、Header 水平铺满整行——搜索框宽度跟随弹层宽度（≈弹框宽减两侧 ~16px 边距），弹层宽度回归由最宽分支行内容驱动（不再为搜索框行多付 ~90px）。焦点守卫、StaysOpenOnClick 防关闭、拖拽捕获守卫等既有机制全部不动。新增守护测试：搜索框铺满所在行、搜索行与分支行等宽、长分支名驱动弹层宽度、搜索框与弹层内容区宽度差 ≤20px。
- **"检查远端同步状态"/"跟踪"子菜单：搜索框无法获得焦点、无法输入（真实 Windows）**：真实桌面实测（SendInput 完整复刻用户流：右键 → 悬停展开 → 键入 → 点击 → 再键入）定位到两条抢焦点路径：(A) 子菜单展开时 MenuBase 自动把焦点给第一个子项——正是搜索框所在行 MenuItem——压掉了 SubmenuOpened 后投递的 searchBox.Focus()；(B) 点击搜索框时 DefaultMenuInteractionHandler.PointerPressed 又把焦点抢给被按的行。键盘输入经主窗口原生焦点路由到共享 FocusManager 的逻辑焦点元素，焦点停在行 MenuItem 上时按键全部落空。叠加陷阱：MenuItem 类处理器把 GotFocus 标记 Handled，普通事件订阅永远收不到该事件（实测行 IsFocused=True 而 GotFocus 订阅零触发）。修复：新增 MenuExtensions.AttachSearchBoxFocusGuard，在搜索框行上以 handledEventsToo 订阅 GotFocus，一旦焦点落入行（含被抢场景）即投递 Background 回调把焦点归还搜索框；"检查远端同步状态"与"跟踪"两处分组菜单均走同一工厂（CreateSearchableRemoteGroupMenuItem）一并修复。真实 Windows 桌面验证：悬停展开 → 搜索框 IsFocused=True、键入 'a' 文本落框、点击搜索框 → 守卫归还焦点、再键入 'b' → 文本='ab'。新增守卫测试（模拟抢焦点后守卫归还）。
- **FileDiff 大区域从下往上拖选（准备暂存）时界面失控弹跳上滚到顶部**：两个叠加缺陷在 `CodeEditor` 层修复。缺陷一（弹跳）：AvaloniaEdit 的 `SelectionMouseHandler` 在 PointerPressed 处理末尾把 `e.Handled` 置 true，CodeEditor 用普通事件订阅挂的"拖选期间抑制 caret 跟随滚动"处理器（订阅晚于包内处理器、且未带 handledEventsToo）从未被触发——拖选期间每次指针移动 caret 变更都触发 `ScrollTo(line, column)` 的"caret 行居中"语义（差超视口 30% 即跳到居中），从下往上拖选时视口逐事件跳升上百像素，指针越过视口上缘后一路弹跳到文档顶部。修复：按下/抬起/移动改用 `AddHandler + handledEventsToo` 挂接（按下走 Tunnel 先于包内处理器完成置位；抬起走 Bubble 晚于包内处理器执行，保证包内 ExtendSelectionOnMouseUp 的 caret 移动仍处于抑制中，抬起本身不再触发居中跳变），移动走 Tunnel 兜底重新置位（捕获中途被夺走又恢复的场景）。缺陷二（首次拖选冻结在两行）：差异视图悬浮 Stage/Discard 按钮首次构建时 `WpfCompat.AdornerLayer` 把窗口内容树整棵摘下重挂，摘除瞬间 Avalonia 将指针捕获挪到最近仍在树的祖先元素（`Pointer.OnCaptureDetached`），拖选的 move/Release 从此不再路由进 TextArea——选区冻结且 Release 收不到、抑制标志悬挂。修复：`PointerCaptureLost` 改按 Direct 路由策略挂接（该事件在 Avalonia 12 注册为 Direct，Bubble 策略处理器收不到）并记录拖选指针，编辑器重挂回可视树时（与摘除同一次同步调用内）若捕获仍空或停在祖先上立即取回，被无关元素（弹窗等）正当持有时不抢。新增守卫测试 `DragSelectAutoScrollProbeTests`：headless 真实鼠标事件两轮从下往上拖选（首轮即触发悬浮按钮/内容树重建），断言单步滚动不超行步进（≤40px，居中跳变约 ±190px）、越过顶部后只按行步进不弹到文档顶部、选区按预期扩展、抑制标志抬起后复位。
- **FileDiff 行间距过宽 + 同区域行数比 WPF 3.13.2 少约 16%（实测 36 行 vs 43 行）**：AvaloniaEdit 行槽高 = max(行自然高, 默认行高 × LineHeightFactor)，两个叠加原因分两步修复。第一步（行间距）：v4.1.2 时含中文的行自然高（Win 21.05px / Linux 18.82px@13px）超默认行槽，SideBySideLineHeightSynchronizer 为保左右对齐把两侧全部行槽统一抬到 CJK 自然高 → 含中文文件行间距过宽；根因是全局回退 Noto Sans CJK SC 垂直度量 1.448em，本版把内嵌子集（Regular + Bold）垂直度量统一收紧（hhea / OS/2 win / OS/2 typo 三处一致）——Linux（FreeType/hhea）、Windows（DirectWrite）、macOS（CoreText）无论读哪张表都收敛到同一行高。第二步（行密度，"36 行 vs 43 行"根因）：AvaloniaEdit 默认 LineHeightFactor=1.16（VS 编辑器风格）把行槽在自然行高之上再放大 16%（Consolas@13 下 15.22 → 17.66px），而 WPF AvalonEdit 行槽就是自然行高 → 同视口可见行数少 16%；CodeEditor / HexEditor 构造显式置 LineHeightFactor=1.0，行槽 = ASCII 自然高（≈15.22px@Win / 15.13px@Linux），行密度与 WPF 3.13.2 一致。第二步联动：默认行槽变矮后 CJK 度量从第一步的 1.25em 再收紧到 1.16em（CJK 自然高 15.08px@13px ≤ 新行槽；否则行高同步器会把含中文文件全部行槽抬到 16.25px，比 WPF 宽 7%）。汉字墨迹（0.88em 上 / 0.12em 下）完整落在新度量内不裁字；33739 个字形中仅 9 个竖排标点（U+3031-3035、U+FFE8）越界且横排文本不使用。行高同步器保留为异常字体度量下的对齐安全网，正常字体下不再抬高行距。新增 / 强化守卫测试：CJK 字体度量不超 factor=1.0 行槽（行间距不回弹）、DefaultLineHeight == ASCII 自然行高（行密度守卫，回归"36 行"症状时在此红）、纯 ASCII 文件 factor 保持 1.0、CJK 混排左右行像素对齐、滚轮后滚动条 thumb 跟随；另修正 DiffEditorScrollExtentProbeTests 期望值按真实渲染行数计算（50 次 AppendLine 实际渲染 51 行——末尾 \n 的幽灵空行，WPF AvalonEdit 同样渲染；此前仅靠浮点舍入误差惊险通过，行高变为二进制可精确表示后翻红）。
- **Windows 上 AI 辅助开发 / AI 解释窗口在程序目录释放 WebView2 目录**：Windows 平台 NativeWebView 的引擎就是真 WebView2（Edge 运行时），此前从未配置用户数据目录——传给 EnsureCoreWebView2Async 的"环境"在兼容层里是空壳（参数无人消费），WebView2 运行时对未打包 Win32 应用回落默认位置 = exe 同级 "<exe名>.WebView2"，每次打开 AI 窗口都在程序目录落目录。修复：WebView2Stub 在创建原生控件时订阅官方 EnvironmentRequested 事件（适配器创建前触发，时序必然覆盖）把 UserDataFolder 固定到 ForkData\WebView2（与降级前 WPF 原版环境参数同位，%LOCALAPPDATA%/ForkPlusData 之下）；非 Windows 平台事件参数类型不同（WPE/WebKitGTK/WKWebView），模式匹配不命中即无副作用。新增守卫测试：重定向契约（args 赋值后必须指向 ForkData\WebView2）+ 用户可见行为（用户数据目录不得与程序目录同树）。
- **Commit 消息组件样式与 WPF 3.13.2 对齐**：三处差异逐一核对修复。清除按钮（ClearCommitMessageButton）的 X 由两条斜线 Line 绘制（12x12 内 3→9），迁移时漏掉 WPF 原版的圆头线帽（StrokeStartLineCap/StrokeEndLineCap=Round），Avalonia 用等价单一属性 StrokeLineCap=Round 补齐（Avalonia Shape 线帽一次设置两端生效，与 WPF 两端 Round 视觉效果一致）。Commit 按钮（CommitButton）垂直 Padding 由 10,0 改回 WPF 原版 10,2（按钮文字上下留白）。其余组件已逐项比对一致无需改动：CommitPlaceholderTextBox（Medium/14px、"3,3,0,0" 圆角、占位 13px Normal）、CommitDescriptionTextBox（Consolas/13px、Padding 4,4,4,2、GuideLine 与占位 Segoe UI）、CommitButtonVisibleDropdownStyle 与 CommitDropDownButtonStyle 的角半径/边框/阴影/箭头/状态色均与 WPF 原版一致。

### 平台覆盖

| 平台 | RID | 产物 |
|------|-----|------|
| Windows x64 | `win-x64` | `ForkPlus-4.1.4-windows-x64.zip` |
| Linux x64 | `linux-x64` | `ForkPlus-4.1.4-linux-x64.zip` |
| Linux ARM64 | `linux-arm64` | `ForkPlus-4.1.4-linux-arm64.zip` |
| macOS ARM64 | `osx-arm64` | `ForkPlus-4.1.4-macos-arm64.zip` |
...
- **打开大文件 / 加载 diff 后视口停在文档末尾而非顶部**：v4.1.2 新装的 caret 跟随滚动（拖动选取回弹修复的配套）被程序化文档替换误触发——Text 替换过程 caret 先临时跳到文档末行再回行首，跟随逻辑按"末行"的陈旧请求把视口滚到了底部（500 行文档赋值后 Offset.Y 即等于 maxOffset，打开即看到文档末尾）。修复为程序化文档替换期间挂起一轮跟随（WPF 原版同语义），键盘 / 鼠标移动 caret 的正常跟随不受影响。
- **首次启动"更新内容"弹窗正文无滚动条（超框内容无法拉取）**：主 TextBox 主题模板的内层 ScrollViewer 滚动条可见性被硬编码为 Hidden（迁移自 WPF 的 PART_ContentHost），弹窗 XAML 上写的 `ScrollViewer.VerticalScrollBarVisibility="Auto"` 被彻底无视——正文超过文本框高度（360px）时既无滚动条也无法拉取。修复为模板 TemplateBinding 到 TextBox 的 ScrollViewer.* 附加属性（默认仍 Hidden，未显式设置的输入框行为不变），同款写法的"检查更新 / 有新版本"弹窗正文、Tag 详情、自定义命令输出、推送 squash 消息、提交模板/正则多行框等一并恢复超框滚动条。
...
- **git mm 子仓空状态在取消/重选时状态错乱（"连续两次选择才正常"）**：空状态判定用 `_workspace.SelectedSubrepo == null`，而它在 `SelectionChanged` 分支内部、判定调用之后才被赋值，读到的一直是过期值——取消选中后 `SelectedSubrepo` 未清（空状态不漏显→内容区空白）；重新点选同个子仓时 `SelectedSubrepo` 仍为 null（空状态残留不消失），必须先选另一个子仓才恢复。修复为：①空状态基准改用 `SubreposTabControl.SelectedItem == null`（事件触发时即为"当前实际选中"的 ground-truth）；②`SelectionChanged` 先落定选中/取消选中状态（取消时同步清空 `SelectedSubrepo`）再统一重算按钮与空状态。
